...
//角度信息 用于存储拨弹盘角度、Yaw角度、Pitch角度
Motor_Angle motor_trigger,motor_gimbal_pitch,motor_gimbal_yaw;
...
void Fdcan1_callback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	else fdcan_RxHeader.DataLength >> 16;
...
void Fdcan2_callback(FDCAN_HandleTypeDef *hfdcan)
{
	...
	switch(fdcan_RxHeader.Identifier)
	{
	  ...
	  case 0x203 : Motor_dat_transmit(&motor_shoot[2], rx_data2);
					       Cale_Trigger_angle_t(&motor_shoot[2], &motor_trigger);
		             break;
//		case 0x205 : Motor_dat_transmit(&motor_gimbal[0], rx_data2);
//								 Cale_Gimbal_angle_t(&motor_gimbal[0], &motor_gimbal_yaw);
//		             break;
//		case 0x206 : Motor_dat_transmit(&motor_gimbal[1], rx_data2);
//								 Cale_Gimbal_angle_t(&motor_gimbal[1], &motor_gimbal_pitch);
//		             break;
...
void Gimbal_cmd_CAN(int16_t motor1, int16_t motor2)
{
	...
	fdcanx_send_data(&hfdcan2, CAN_GIMBAL_ALL_Control, Dat_g);
}
